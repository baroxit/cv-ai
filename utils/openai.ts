...
export async function createCV() {
    ...
    const { data: experiences, error: experiencesError } = await supabase
        .from('experiences') // Assuming your experiences table is called 'experiences'
    ...
    // Add company data from argument
    const jobPosition = {
    ...
    const finalData = { cv_data: cvData, job_position: jobPosition };
    ...
    const dataToUpload = {
        ...
        work_experiences: object.work_experiences,
    ...
    const { error } = await supabase.from("generated_cvs").insert(dataToUpload);
...
export async function generateDescription(prompt, currentDesc) {
