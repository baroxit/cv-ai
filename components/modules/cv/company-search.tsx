...
const CvCompanySearch: React.FC<CompanySearchComboboxProps> = ({ onChange, label = '' }) => {
		...
		async (query: string) => {
			...
			try {
				const response = await fetch(
					`https://api.brandfetch.io/v2/search/${query}?limit=5&c=${process.env.NEXT_PUBLIC_BRANDFETCH_API_KEY}`,
					{ method: 'GET' }
				)
								...
								<>
									<CommandGroup>
										{results.map((result) => (
											<CommandItem
												key={result.domain}
												value={result.name + result.domain}
											...
											>
												...
												<Check
													className={cn('ml-auto', result.domain === value?.domain ? 'opacity-100' : 'opacity-0')}
												/>
											</CommandItem>
										))}
									</CommandGroup>
									<div className='hover:bg-card' onClick={handleManualAdd}>
