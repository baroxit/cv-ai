...
function DisplayCard({
	...
	return (
		<div
			className={cn(
				"relative h-22 w-[22rem] -skew-y-[8deg] select-none flex-col justify-between rounded-xl border bg-muted/70 backdrop-blur-sm px-4 py-4 transition-all duration-700 after:absolute after:-right-1 after:top-[-5%] after:h-[110%] after:w-[20rem] after:bg-gradient-to-l after:from-neutral-950 after:to-transparent after:content-[''] hover:border-white/20 hover:bg-muted [&>*]:flex [&>*]:items-center [&>*]:gap-2",
				className
			)}
		>
			<div className='items-center flex'>
				<Avatar className='h-11 w-11 rounded-lg'>
					<AvatarImage
						src={`https://cdn.brandfetch.io/${domain}/w/400/h/400?c=${process.env.NEXT_PUBLIC_BRANDFETCH_API_KEY}`}
						alt=''
					/>
					<AvatarFallback className='rounded-lg uppercase'>{company.slice(0, 2)}</AvatarFallback>
				</Avatar>
				<div className='ml-1'>
					<p className={cn('text-lg font-medium mb-[-7px]')}>
						Giacomo {' <> '} <span className='capitalize'>{company}</span>
					</p>
