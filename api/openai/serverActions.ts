...
export async function createCv(formData: FormData) {
	...
	try {
		...
		throw new Error(error.message || 'An unexpected error occurred while creating the CV.')
	}
}
...
export async function uploadCV(file: File) {
	try {
		...
		if (error.message.includes('file')) {
			throw new Error('The uploaded file could not be processed. Please ensure it is a valid PDF.')
		}

		throw new Error(error.message || 'An unexpected error occurred while processing the CV.')
	}
}
