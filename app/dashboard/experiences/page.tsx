				...
				<Tabs defaultValue='work-experiences'>
					...
					<TabsContent value='work-experiences'>
						<div className={`grid auto-rows-min gap-4 ${experiences && experiences?.length > 0 && 'md:grid-cols-2'}`}>
							...
							{experiences?.length == 0 && <DropzoneCv />}
					...
					<TabsContent value='education'>
						<div
							className={`grid auto-rows-min gap-4 ${
								education_experiences && education_experiences?.length > 0 && 'md:grid-cols-2'
							}`}
						>
							{education_experiences &&
								education_experiences.map((education_experience) => (
									<EducationCard key={education_experience.id} education={education_experience} />
								))}
							<Card className='flex flex-col justify-center items-center text-center'>
								<CardHeader>
									<CardTitle>Add a new experience</CardTitle>
					...
					<TabsContent value='projects'>
						<Card className='max-w-[350px] mt-6 text-center mx-auto'>
							<CardHeader>
								...
								<CardDescription>
									<p className='mb-2'>I am still working on this area.</p>{' '}
